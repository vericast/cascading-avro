...
public class AvroScheme extends AvroSchemeBase {
	...
	@SuppressWarnings("unchecked")
	@Override
	public void sink(TupleEntry tupleEntry, OutputCollector output)
			throws IOException {
		...
		output.collect(new AvroWrapper<IndexedRecord>(record),
				NullWritable.get());
	}

	private Object toAvro(Comparable field, FieldType typeInfo, Object val)
			throws IOException {
		if (val == null) {
